...
function PaginationControls({ page, pageSize, total, disabled, onPageChange }: { page: number; pageSize: number; total: number; disabled?: boolean; onPageChange: (p: number) => void }) {
    ...
    return <div className="flex items-center gap-2 justify-end mt-4">
        ...
        <ul className="flex items-center gap-1">{pages.map((p, i) => p === '…' ? <li key={`ellipsis-${i}`} className="px-1 text-xs text-muted-foreground">…</li> : <li key={`page-${p}`}><button disabled={disabled || p === page} onClick={() => go(p)} aria-current={p === page ? 'page' : undefined} className={`h-8 w-8 rounded-md text-xs border ${p === page ? 'bg-accent font-medium' : 'hover:bg-accent/60'}`}>{p}</button></li>)}</ul>
